...
import math
import sys
from python_vehicle_simulator.lib.control import integralSMC, S_surface_control, S_surface_control_nref, PIDcontrol_nref
...
class remus100:
    ...
    def __init__(
        ...
        self.r_d = 0
        self.a_d = 0
        self.wn_d = 0.8    # 0.8               # desired natural frequency
        self.zeta_d = 1.0 # 1.0                   # desired realtive damping ratio 
        self.r_max = 13.5 * math.pi / 180 # 13.5   # maximum yaw rate 13.5
        ...
        # Depth autopilot
        self.wn_d_z = 0.15     # desired natural frequency, reference model
        self.Kp_z = 0.1        # heave proportional gain, outer loop
        self.T_z = 100       # heave integral gain, outer loop
        self.Kp_theta = 6.0    # pitch PID controller     
        self.Kd_theta = 2.0  
        self.Ki_theta = 0.05
    ...
    def depthHeadingAutopilot(self, eta, nu, sampleTime, control='SMC', k_yaw = [2, 1, 0.01], k_depth = [1, 1, 0.01]):
        """
        ...
        e_psi = self.angprocess(psi - self.psi_d) if control[-2:] != 'nv' else self.angprocess(psi - psi_ref)  # self.angprocess(psi - self.psi_d)  # yaw angle tracking error
        ...
        # PI controller (PVS Default)
        theta_d = self.Kp_z * ( (z - z_ref) + (1/self.T_z) * self.z_int ) # z - self.z_d
        ...
        error = z - z_ref
        try:
            error_d = (error - self.old_error) / sampleTime
        except:
            error_d = 0

        constrain = lambda x_min, x, x_max: x if (x_min <= x <= x_max) else (x_min if (x < x_min) else x_max)

        if control.find('Ssurface') != -1:
            delta_s = 2 / (1 + np.exp(-k_depth[0] * error - k_depth[1] * error_d)) - 1 # k_depth[2] and k_yaw[2] are dummy if the control method is 'Ssurface'
        elif control.find('PID') != -1:
            delta_s = k_depth[0] * error + k_depth[1] * error_d + k_depth[2] * constrain(-20,self.z_int,20)
        ...
        elif control == 'Ssurface': # ref model 
            [delta_r, self.e_psi_int, self.psi_d, self.r_d, self.a_d] = \
                S_surface_control(
                    self.e_psi_int, 
                    e_psi, e_r, 
                    ...
                    zeta_d, 
                    self.r_max,
                    sampleTime,k1=k_yaw[0],k2=k_yaw[1]
                )
        elif control.find('Ssurface_nref') != -1: # w/o ref model
            [delta_r, self.e_psi_old, self.psi_d, self.r_d, self.a_d] = \
                S_surface_control_nref(
                    self.e_psi_old, 
                    e_psi, e_r, 
                    self.psi_d, 
                    self.r_d, 
                    ...
                    self.r_max,
                    sampleTime,
                    mode=control,k1=k_yaw[0],k2=k_yaw[1]
                )
        elif control.find('PID_nref') != 1:
            [delta_r, self.e_psi_old, self.e_psi_int, self.psi_d, self.r_d, self.a_d] = \
                PIDcontrol_nref(
                    self.e_psi_int,
                    self.e_psi_old, 
                    e_psi, e_r, 
                    ...
                    self.r_max,
                    sampleTime,
                    mode=control,k_p=k_yaw[0], k_d=k_yaw[1], k_i=k_yaw[2]
                )
        u_control = np.array([ delta_r, -delta_s, n], float)
