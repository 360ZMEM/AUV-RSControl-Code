...
def S_surface_control(e_int, e_x, e_v, x_d, v_d, a_d, r, wn_d, zeta_d, v_max, sampleTime, k1=2, k2=1):
    delta = 2 / (1 + np.exp(k1 * e_x + k2 * e_v)) - 1
    e_int += sampleTime * ssa(e_x)
    [x_d, v_d, a_d] = refModel3(x_d, v_d, a_d, r, wn_d, zeta_d, v_max, sampleTime)
    return delta, e_int, x_d, v_d, a_d

def S_surface_control_nref(e_prev, e_x, e_v, x_d, v_d, a_d, r, wn_d, zeta_d, v_max, sampleTime,  k1=2, k2=1, mode='nv'):
    if mode[-2:] == 'nv': 
        e_v = ((e_x - e_prev) / sampleTime) if (e_prev is not None) else 0
    delta = 2 / (1 + np.exp(k1 * e_x + k2 * e_v)) - 1
    #e_int += sampleTime * ssa(e_x)
    return delta, e_x, x_d, v_d, a_d 

def PIDcontrol_nref(e_int, e_prev, e_x, e_v, x_d, v_d, a_d, r, wn_d, zeta_d, v_max, sampleTime, k_p=2, k_d=1, k_i=0.01, int_max=1.8, mode='nv'): # 
    if mode[-2:] == 'nv': 
        e_v = ((e_x - e_prev) / sampleTime) if (e_prev is not None) else 0
    delta = - k_p * e_x - k_d * e_v - k_i * e_int
    e_int = constrain(-int_max, e_int + sampleTime * ssa(e_x), int_max)
    return delta, e_x, e_int, x_d, v_d, a_d